...
for i in range(n_imp):
	...
	for j in range(10):
		clf = RandomForestClassifier(n_estimators=100,criterion="gini",max_features="auto",max_depth=3, random_state=j, n_jobs=6)
		clf_isotonic = CalibratedClassifierCV(clf, method='isotonic')
		res = clf_isotonic.fit(datRF_sep_train.iloc[:,1:], datRF_sep_train.iloc[:,0])
...
dat_info_test.to_csv("YCU_PJI_preope_RFprob_calibrated_isotonic.txt",sep="\t")
